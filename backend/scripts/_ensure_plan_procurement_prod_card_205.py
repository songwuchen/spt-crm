...
OUT = ROOT / "_ensure_plan_procurement_prod_card_205_out.txt"
...
from app.common.rbac_sync import ensure_plan_procurement_dept_role
...
async def main():
    ...
    async with async_session_factory() as db:
        res = await ensure_plan_procurement_dept_role(db, tid)
        await db.commit()
        await invalidate_tenant_auth_cache(tid)
        role = (await db.execute(
            select(Role).where(Role.tenant_id == tid, Role.code == "plan_procurement_dept")
        )).scalar_one_or_none()
        members = []
        perms = []
        if role:
            ...
            perms = (await db.execute(text("""
                ...
                WHERE rp.role_id = :rid
                  AND p.code IN ('form_data:view','order:view','product:view')
                ORDER BY p.code
...
def main() -> None:
    ...
    with sftp.file("/tmp/_ensure_plan_procurement_prod_card_run.py", "w") as f:
        f.write(INNER)
    sftp.close()

    run(f"{compose} cp /tmp/_ensure_plan_procurement_prod_card_run.py backend:/tmp/_ensure_plan_procurement_prod_card_run.py")
    code, out = run(
        f"{compose} exec -T backend sh -c 'cd /app && PYTHONPATH=/app python /tmp/_ensure_plan_procurement_prod_card_run.py'"
    )
