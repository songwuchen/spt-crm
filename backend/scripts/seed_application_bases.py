"""安装应用领域/应用物料内置表单，并从 docs dump 导入简道云选项为表单实例。

Idempotent：同名 name 已存在则跳过。

Run:
  python -m scripts.seed_application_bases
  python -m scripts.seed_application_bases <tenant_id>
"""
...
FORMS = (
    ("application_field", "_jdy_application_field_options.json"),
    ("application_material", "_jdy_application_material_options.json"),
)

JDY_NAME_FIELD = "_widget_1595923157958"


def _load_names(options_file: str) -> list[str]:
    p = DOCS / options_file
    if not p.exists():
        data_p = DOCS / options_file.replace("_options.json", "_data.json")
        if not data_p.exists():
            raise FileNotFoundError(f"missing {p} (run scripts._pull_jdy_application_bases first)")
        ...
        for r in rows:
            if not isinstance(r, dict):
                continue
            n = str(r.get(JDY_NAME_FIELD) or r.get("label") or r.get("name") or "").strip()
            if n:
                names.append(n)
        return _uniq(names)
...
async def seed_tenant(db, tenant_id: str) -> dict:
    ...
    print(f"[{tenant_id[:8]}] initiator={user_id}")
    stats: dict = {}
    for code, opt_file in FORMS:
        names = _load_names(opt_file)
