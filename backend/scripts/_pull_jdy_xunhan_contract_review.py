#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Pull JDY「迅焊公司合同评审」fields + workflow + edit_raw into docs/product."""
...
ENTRY = "67d3d515c8df85cc24de064f"  # 迅焊公司合同评审
KEY_NAME = "xunhan_contract_review"
TITLE = "迅焊公司合同评审"
...
def main() -> None:
    ...
    print("pull edit …")
    try:
        edit = get_json(f"{BASE}/api/form/{APP}/{ENTRY}/edit")
        (OUT / f"_jdy_{KEY_NAME}_edit_raw.json").write_text(
            json.dumps(edit, ensure_ascii=False, indent=2), encoding="utf-8"
        )
        print("edit ok")
    except Exception as e:
        print(f"edit fail: {e}", file=sys.stderr)

    md = [
        f"# {TITLE}（字段对照）",
        "",
        f"> 简道云 app=`{APP}` entry=`{ENTRY}`（销售中心 / 合同 / 迅焊公司合同评审）。",
        f"> CRM builtin code: `{KEY_NAME}`。",
