...
from app.domains.lowcode.workflow_service import (
    ...
    _contract_review_legal_sup_user_aligned,
    apply_contract_review_named_legal_approvers,
    apply_legal_sup_named_approver,
)


def test_contract_review_legal_sup_is_yuanwenjun():
    nodes, _ = _contract_review_flow_graph()
    by_id = {n["id"]: n for n in nodes}
    rule = by_id["approval_legal_sup"]["approver_rule"]
    assert rule["type"] == "specified_user"
    assert rule["value"] == "02364840011125"
    assert _contract_review_legal_sup_user_aligned(nodes)
...
def test_apply_legal_sup_named_approver():
    nodes = [{
        "id": "approval_legal_sup",
        "name": "法务主管审批",
        "approver_rule": {"type": "specified_user", "value": "492105073721398323"},
    }]
    assert apply_legal_sup_named_approver(nodes)
    assert nodes[0]["approver_rule"]["value"] == "02364840011125"
    assert not apply_legal_sup_named_approver(nodes)
