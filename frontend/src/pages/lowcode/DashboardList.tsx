...
export default function DashboardList() {
  ...
  const load = async () => {
    setLoading(true)
    try { const r = await dashboardApi.list({ pageNo: 1, pageSize: 100 }); setItems(r.data.items) } finally { setLoading(false) }
  }
  useEffect(() => { load() }, [])
  ...
  const del = async (id: string) => { await dashboardApi.remove(id); message.success('已删除'); load() }

  return (
    <Card loading={loading}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <Title level={4} style={{ margin: 0 }}>仪表盘</Title>
        <Button type="primary" icon={<PlusOutlined />} onClick={() => setOpen(true)}>新建仪表盘</Button>
      </div>
      {items.length === 0 ? <Empty description="还没有仪表盘,点右上角新建" /> : (
        <Row gutter={[16, 16]}>
          {items.map((d) => (
      ...
      <Modal title="新建仪表盘" open={open} onOk={create} onCancel={() => setOpen(false)} destroyOnClose>
