        ...
        <div className="space-y-2">
          ...
          {data.map((c) => (
            <SwipeAction
              key={c.id}
              actions={[
                { label: '拨号', color: '#3b82f6', onClick: () => handleCall(null) },
                { label: '删除', color: '#ef4444', onClick: () => {
                  customerApi.delete(c.id).then(() => { message.success('已删除'); fetchData() })
                }},
              ]}
            ...
            >
              <div
                ...
                className={`p-3 flex items-center gap-3 active:bg-slate-50 cursor-pointer border border-slate-100 ${selectedIds.has(c.id) ? 'bg-blue-50' : 'bg-white'}`}
              >
