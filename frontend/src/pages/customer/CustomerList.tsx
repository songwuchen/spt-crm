  ...
  const columns: ColumnsType<Customer> = [
    ...
    ...([
      { title: '国家/地区', dataIndex: 'country', key: 'country', width: 100,
        render: (v: unknown) => textCell(v) },
      { title: '客户代码', dataIndex: 'foreign_customer_code', key: 'foreign_customer_code', width: 110,
        render: (v: unknown) => textCell(v) },
      { title: '外贸客户类型', dataIndex: 'foreign_customer_type', key: 'foreign_customer_type', width: 110,
        render: (v: unknown) => textCell(v) },
      { title: '关注产品', dataIndex: 'focus_product', key: 'focus_product', width: 110,
        render: (v: unknown) => textCell(v) },
      { title: '邮箱', dataIndex: 'customer_email', key: 'customer_email', width: 160,
        render: (v: unknown) => textCell(v) },
      { title: '主页', dataIndex: 'website', key: 'website', width: 140,
        render: (v: string) => v
          ...
          : dash },
      { title: '主营产品', dataIndex: 'main_products_json', key: 'main_products_json', width: 140,
        render: (v: string[] | null) => Array.isArray(v) && v.length
          ...
          : dash },
      { title: '企业法人', dataIndex: 'legal_person', key: 'legal_person', width: 100,
        render: (v: unknown) => textCell(v) },
      { title: '员工人数', dataIndex: 'headcount', key: 'headcount', width: 90, align: 'right' as const,
        render: (v: number | null) => numCell(v) },
      { title: '业务部门', dataIndex: 'department_name', key: 'department_name', width: 120,
        render: (v: unknown) => textCell(v) },
      { title: '行业分类', dataIndex: 'smart_industry_category', key: 'smart_industry_category', width: 120,
        render: (v: unknown) => textCell(v) },
      { title: '年运行天数', dataIndex: 'annual_run_days', key: 'annual_run_days', width: 100,
        render: (v: unknown) => textCell(v) },
      { title: '占地面积', dataIndex: 'floor_area', key: 'floor_area', width: 100,
        render: (v: unknown) => textCell(v) },
      { title: '年用电量', dataIndex: 'annual_power_usage', key: 'annual_power_usage', width: 100,
        render: (v: unknown) => textCell(v) },
      { title: '日运营小时', dataIndex: 'daily_operate_hours', key: 'daily_operate_hours', width: 100,
        render: (v: unknown) => textCell(v) },
      { title: '是否公司客户', dataIndex: 'is_company_customer', key: 'is_company_customer', width: 110,
        render: (v: boolean | null) => ynTag(v) },
      { title: '纳税人识别号', dataIndex: 'taxpayer_id', key: 'taxpayer_id', width: 180,
        render: (v: unknown) => textCell(v) },
      { title: '开票地址电话', dataIndex: 'invoice_address_phone', key: 'invoice_address_phone', width: 160,
        render: (v: unknown) => textCell(v) },
      { title: '开户行帐号', dataIndex: 'bank_account', key: 'bank_account', width: 160,
        render: (v: unknown) => textCell(v) },
      { title: '规模等级', dataIndex: 'scale_level', key: 'scale_level', width: 90,
        render: (v: unknown) => textCell(v) },
      { title: '采购意向', dataIndex: 'intent_level', key: 'intent_level', width: 92,
        render: (v: string) => v
          ? <Tag color={intentLevelColors[v] || 'default'} className="m-0">{intentLevelShortLabels[v] || v}</Tag>
          : dash },
      { title: '标签', dataIndex: 'tags_json', key: 'tags_json', width: 150,
      ...
      { title: '结单', dataIndex: 'won_deal_count', key: 'won_deal_count', width: 64, align: 'center' as const,
        render: (v: number) => v ? <span className="font-semibold text-emerald-600">{v}</span> : <span className="text-slate-300">0</span> },
      { title: '录入人', dataIndex: 'created_by_name', key: 'created_by_name', width: 100,
        render: (v: unknown) => textCell(v) },
    ] as ColumnsType<Customer>).map((c) => ({ ...c, __optIn: true })),
