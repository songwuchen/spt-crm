...
import { leadReactivationFilterOptions, leadReactivationStatusConfig } from '@/constants/labels'
...
function flowStatusLabel(row: LeadReactivationRecord) {
  if (row.is_current_round && row.reactivation_status && row.reactivation_status !== 'none') {
    const cfg = leadReactivationStatusConfig[row.reactivation_status]
    if (cfg) return cfg
  }
  if (row.flow_status === 'completed') return { label: '流程结束', tone: 'slate' }
  if (row.flow_status === 'closed') return { label: '已关闭', tone: 'slate' }
  if (row.flow_status === 'running') return { label: '进行中', tone: 'blue' }
  return { label: row.flow_status || '-', tone: 'slate' }
}
...
export default function LeadReactivationList() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      } else if (statusFilter && statusFilter !== '__all__') {
        params.reactivation_status = statusFilter
      }
      const res = await leadReactivationApi.list(params)
  ...
  const reloadList = () => { void fetchData() }
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500 mt-0.5">
          每条记录对应简道云一条「180天项目激活」数据，可查看原申报信息与流程动态
        </p>
      </div>
