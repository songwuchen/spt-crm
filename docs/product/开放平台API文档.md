...
| `review_status` | string | 否 | 审核态。对接简道云「项目最终状态」：`收录`/`approved`→已通过（**免 CRM 情报审**）；`袭击`/`attacked`；`回退`/`rejected`；`待审`/`draft` 或未传→草稿 |
...
> **审核态**：简道云已收录/袭击/回退的线索按上表落终态，不再强制草稿；未审或待审仍落草稿，便于幂等补全后再在 CRM 提交情报审。
示例：
```bash
