...
| field_3 | 类型 | radio | 是 | 是 | initiator | `_widget_1703723955300` |
| customer_name | 客户名称 | customer | 是 | 是 | initiator | `_widget_1577514157595` |
| field_4 | 业务部门 | department | 是 | 是 | initiator | `_widget_1578119863142` |
| sales_person | 业务员 | person | 是 | 是 | initiator | `_widget_1577520412656` |
| field_5 | 现场联系人及电话 | text | 是 | 是 | initiator | `_widget_1577514157633` |
| field_6 | 货物地址 | address | 是 | 是 | initiator | `_widget_1577514157611` |
...
| └ field_8 | 设备名称 | text | 是 | | | `_widget_1577519022538` |
| └ field_9 | 规格型号 | text | 是 | | | `_widget_1577519022539` |
| └ field_10 | 数量 | number | 是 | | | `_widget_1577519022540` |
| └ field_11 | 单位 | text | 是 | | | `_widget_1578287861127` |
| └ field_12 | 发货日期 | datetime | 是 | | | `_widget_1577519022541` |
| └ field_13 | 退回产品详细说明 | textarea | 是 | | | `_widget_1577519022542` |
| └ remark_2 | 备注 | textarea |  | | | `_widget_1577519022543` |
| └ field_14 | 仓库判定* | radio | 是 | | | `_widget_1665817650935` |
...
| field_23 | 维修部门 | department |  |  | approver | `_widget_1578128358033` |
| f_1 | 仓库判定1 | select |  |  | approver | `_widget_1578127027977` |
| field_24 | 流程判断 | radio |  |  | approver | `_widget_1691974761704` |
| field_25 | 物流情况 | radio |  |  | approver | `_widget_1753923897787` |
| field_26 | 是否转相关人员 | radio |  | 是 | initiator | `_widget_1734925951092` |
| field_27 | 转相关人员 | person_multi | 是 | 是 | initiator | `_widget_1734925951094` |

### 流程降级备注

- 提交人/发起部门：发起默认当前用户及主部门
- 明细「仓库判定*」：仅审批节点可填（发起不展示）
- 审批「仓库接收1」具名用户 02366368263850，无匹配用户时 auto_approve
- 审批「客服办理/会签」JDY 角色「230902客服内勤」→ 指定用户 ['0236446249514', '181359282120075679', '113236314224043072', '01364955133227249077']
...
- optAuth：10 个字段仅审批可写（创建 available_on_create=false，必填下沉到节点 field_perms）
- optAuth：3 个字段发起仅可见（form_editable=false）
- edit_raw allowBlank=false：23 个必填 widget
- fieldShowRules → 3 条显隐/条件必填规则
